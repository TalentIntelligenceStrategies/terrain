...
3. **Who supplies the per-patent "open this in the source" URL?** The source varies by jurisdiction —
   ...
   **The affordance is now built and its destination is a stand-in.** *Open in IPtech* sits in the
   record's head and points every patent at the same sign-in page. It is deliberately not a
   constructed link: a per-patent URL assembled from a number and a guessed registry would be right
   often enough to be trusted and wrong without saying so. Two fields on the record — a `url` and a
   `label` — close this, and until they arrive the control is honest about being a placeholder
