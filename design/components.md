...
| **The grouping panel** · `cluster` | `--border` `--text-1/2/3`, no accent, **no caption** | **request** `{ setId }` · **response** `{ head: string, spines: [{ label, leaves: [{ id, label, ids: [patentId] }] }] }` — **two levels and no more.** `ids` is what the interface filters the list by, so the grouping must partition the set it was given rather than re-query. Below a floor it returns no spines and the panel says there are too few results to group | **AI魚骨**, pointed at a result set. **Question 2 in §4** — the whole panel depends on an answer we do not have |
